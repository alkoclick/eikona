...
class LoginPage : BootstrapPageTemplate {
    ...
    override val additionalCssHref = "/assets/css/pages/login.css"

    override fun renderBody(body: BODY) {
        body.main("container form-signin w-100 m-auto") {
            form {

                h2(classes = "h2 display-2 text-center link-primary eikona-title") {
                    text("Eikona")
                }
                h4(classes = "h4 mb-5 display-4 text-center") {
                    text("🔒 📷 ☁️")
                }
                ...
                div(classes = "form-floating mb-3") {
                    input(classes = "form-control", type = InputType.email) { id = "floatingInput"; placeholder = "name@example.com" }
                    label { htmlFor = "floatingInput"; text("Email Address") }
                }

                div(classes = "form-floating mb-3") {
                    input(classes = "form-control", type = InputType.password) { id = "floatingPassword"; placeholder = "Password" }
                    label { htmlFor = "floatingPassword"; text("Password") }
                }
...
inline fun FlowOrPhrasingContent.path(classes: String? = null, d: String? = null, fill: String? = null, crossinline block: Path.() -> Unit = {}): Unit =
    Path(attributesMapOf("class", classes, "d", d, "fill", fill), consumer).visit(block)

open class Path(initialAttributes: Map<String, String>, override val consumer: TagConsumer<*>) : HTMLTag("path", consumer, initialAttributes, null, inlineTag = false, emptyTag = false),
    HtmlBlockInlineTag
